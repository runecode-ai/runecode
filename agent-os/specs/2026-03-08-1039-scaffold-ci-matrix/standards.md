...
- If CI includes both lockfile-specific and repository-wide cleanliness checks, run lockfile-specific checks first so lockfile failures are not masked by broader checks.
...
- Keep `.gitattributes` rules minimal and non-redundant; prefer `* text=auto eol=lf` and add file-type overrides only for explicit exceptions.
